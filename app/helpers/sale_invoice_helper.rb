		...
		begin
			StockTransaction.transaction do
				data.invoice_detail.each_with_index do |d|
					item = d.item_sku
					cost_avc = (Items.find(d.item_sku.item_id)).cost_avc
					...
					data.input_cost= cost_avc
					data.avg_cost=  cost_avc
					data.last_value= (remain_qty * cost_avc)
					data.total_value=  (total_qty * cost_avc)
