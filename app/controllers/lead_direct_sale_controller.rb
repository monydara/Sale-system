class LeadDirectSaleController < ApplicationController
	@@common = Common.new
	def index
		...
		if user_permission==true
			data = Lead.joins(:area, :contact,:look_up, :sale_representative, :source).where("leads.flag=#{2}").select("leads.* , areas.name 'area_name',
		 ...
		 look_ups.name status_name, concat(sale_representatives.first_name,' ',sale_representatives.last_name) as leadowner, sources.name as source_name")
			...
			data = Lead.joins(:area, :contact,:look_up, :sale_representative, :source).where("leads.created_by=#{user_id} and leads.flag=#{2}").select("leads.* , areas.name 'area_name',
		 leads.contact_id contact , contacts.contact_name , contacts.contact_mobile ,
		 look_ups.name status_name, concat(sale_representatives.first_name,' ',sale_representatives.last_name) as leadowner, sources.name as source_name ")
		end
